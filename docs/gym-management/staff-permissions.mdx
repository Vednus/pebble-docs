...
- **Employees** - Employees only have permission to view any Pebble Connect scanners that have issues or any climbs that are missing scanners. 
...
Below is a breakdown of actions on Pebble that are restricted to specific user permissions and the permissions allowed to perform that action.
<PermissionsTable />